...
export default function MyShift() {
  ...
  const handleDeleteResident = async (id: string, name: string) => {
    ...
    try {
      ...
      alert(err.message || 'Failed to delete resident profile.');
    }
  };
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const { data: resData, error: resError } = await supabase
        .from('residents')
        .select('id, name, room_number, care_level, wing_id')
        .eq('facility_id', facility.id)
        .eq('is_active', true)
    ...
    <div className="min-h-screen bg-[#ffffff] dark:bg-[#0b0b0d] text-[#1f1f1f] dark:text-[#e3e3e3] flex flex-col pb-16 transition-colors duration-300 font-sans">
      ...
      <header className="sticky top-0 z-40 bg-[#ffffff]/80 dark:bg-[#0b0b0d]/80 backdrop-blur-md border-b border-[#e3e3e3] dark:border-[#202024] px-6 py-3.5 transition-colors duration-200">
        <div className="max-w-6xl mx-auto flex items-center justify-between">
          ...
          <nav className="hidden md:flex items-center gap-6 text-[13px] font-medium text-slate-500 dark:text-slate-400">
            <span className="text-[#1f1f1f] dark:text-[#ffffff] border-b-2 border-[#1f1f1f] dark:border-[#ffffff] pb-1 cursor-default">
              Shift Registry
            </span>
            <Link href="/tasks" className="hover:text-[#1f1f1f] dark:hover:text-[#ffffff] transition-colors pb-1">
              Shift Tasks
            </Link>
            <Link href="/" className="hover:text-[#1f1f1f] dark:hover:text-[#ffffff] transition-colors pb-1">
              Live Dashboard
            </Link>
          </nav>
      ...
      <main className="max-w-5xl mx-auto w-full px-6 mt-12 flex-1 flex flex-col">
        ...
        <div className="space-y-6 mb-10">
          ...
          <div className="flex border-b border-[#e3e3e3] dark:border-[#202024] gap-6 text-[13px] font-semibold text-slate-400 dark:text-slate-500">
